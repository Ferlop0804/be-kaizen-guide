...
function validateBody(body: any) {
  ...
  if (body.challenge && body.challenge.length > 1000) errors.push("Mensaje demasiado largo");
  return errors;
}
...
const handler: Handler = async (event) => {
  ...
  try {
    ...
    const { fullName, company, email, challenge } = body;
    ...
    await sendEmail({
      ...
      html: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px">
        ...
        <p><strong>Mensaje:</strong><br/>${challenge || "No especificado"}</p>
        <hr style="margin:16px 0"/>
        <p style="color:#666"><em>Acción recomendada: Contactar dentro de las próximas 24 hs.</em></p>
      </div>`,
    });

    console.log("Demo request processed, emails sent");
